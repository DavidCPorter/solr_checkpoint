...
function start_experiment() {
    ...
    pscp -l $USER -r -h $PROJECT_HOME/ssh_files/pssh_traffic_node_file_6 $PY_SCRIPT /users/dporte7
    pscp -l $USER -h $PROJECT_HOME/ssh_files/pssh_traffic_node_file_6 $TERMS /users/dporte7
    ...
    pssh -h $PROJECT_HOME/ssh_files/pssh_traffic_node_file_6 --user $USER "rm ~/traffic_gen/http_benchmark_*"
...
########## EXPERIMENT LOOPS #################
    ...
    if [ ${19} = '1' ]; then
      ...
      pscp -l $USER -h $PROJECT_HOME/ssh_files/pssh_traffic_node_file_5 ./remotescript.sh /users/$USER/traffic_gen
      pscp -l $USER -h $PROJECT_HOME/ssh_files/pssh_traffic_node_file_single ./remotescript_foreground.sh /users/$USER/traffic_gen
      nohup pssh -l $USER -h $PROJECT_HOME/ssh_files/pssh_traffic_node_file_5 "cd $(basename $PY_SCRIPT); bash remotescript.sh"&
